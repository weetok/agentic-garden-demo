...
Status: Evening 1 complete (blocks 1.1–1.4 committed). Contracts frozen;
dashboard shell, overview (bed map + metrics + advice), and briefing tab all
render live from hand-typed stub JSON (`docs/data/derived.json`,
`docs/data/briefing.md`, `docs/data/planting_map.json`). Block 2.1 done:
`data/sensor_series.json` (3-week hourly site series, generated by
`pipeline/generate_mock_data.py`) and `data/weather_regional.json`
(Open-Meteo-shaped, past_days+forecast_days in one response) carry the
drought story — week 1 normal, weeks 2–3 heat/wind building, final week site
gauge 2.0 mm vs regional 10.0 mm, outlook forecast 4.0 mm. Dashboard still
reads the Evening-1 stub JSON; block 2.2 (Hargreaves derived layer) is next
and will replace it with pipeline output.
